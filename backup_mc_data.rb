...
REMOTE_SSH_KEYFILE = '/home/user/.ssh/id_rsa'
...
def archive_directory(dir_to_backup)
  ...
  `tar cf #{$server_backup_filename} #{dir_to_backup}`
  puts $?.exitstatus
  if $?.exitstatus != 0
    error_exit "Tar failed to create file."
  end
  `rm #{$server_backup_filename}.gz` # prevent 'do you want to overwrite' message
...
def archive_database(user, pass, name, host)
  `mysqldump -u#{user} -p#{pass} -h#{host} #{name} > #{$mysql_output_filename}`
  if $?.exitstatus != 0
    error_exit 'Mysqldump failed.'
  end
  `rm #{$mysql_output_filename}.gz`
...
def sync_with_remote
  puts "Beginning sync with remote server #{REMOTE_SSH_HOST}"
  `rsync . #{REMOTE_SSH_USER}@#{REMOTE_SSH_HOST}:backups/`
  if $?.exitstatus != 0
